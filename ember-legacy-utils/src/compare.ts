import type { TypeName } from './type-of.ts';
...
/**
 Compares two javascript values and returns:
  ...
  ```javascript
  import { compare } from '@ember/legacy-utils';
...
export default function compare<T>(v: T, w: T): Compare {
  ...
  switch (type1) {
    ...
    case 'array': {
      ...
      const vLen = v.length;
      const wLen = w.length;
      const len = Math.min(vLen, wLen);

      for (let i = 0; i < len; i++) {
        const r = compare(v[i], w[i]);
        if (r !== 0) {
          return r;
        }
      }
...
function isComparable(
  value: unknown,
): value is Comparable & ComparableConstructor {
